...
- Monthly estimate: one bar per used model (no tier splits). Bar fill = official monthly usage %; max label = the model's monthly dollar allowance (e.g. MiMo \, DeepSeek \); dotted = projected usage at reset with the total 7-day request pace (all non-free models) priced at that row model's per-request cost (actual 7d avg, else window avg, else snapshot pattern), divided by the overall pool (window spend of non-free models / official usage %); right label = projected spend by reset, red cap date (en-GB) when the projection crosses 100%. GLM Flash (latest) reference row (\). big-pickle/zen and snapshot free models excluded.
