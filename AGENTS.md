...
- Monthly estimate: one bar per used model (no tier splits). Bar fill = official monthly usage %; max label = the model's monthly dollar allowance (e.g. MiMo \, DeepSeek \); dotted = projected overall usage at reset if that model's 7-day spend continues, measured against the overall pool (window spend of non-free models / official usage %); right label = that model's projected spend by reset, red cap date when the projection crosses 100% of the pool. GLM Flash (latest) reference row (\). big-pickle/zen and snapshot free models excluded.
